...
.LFB0:
	.cfi_startproc
	endbr64	
# /tmp/tmp_ed5j6ol.c:4:     for (int i = 0; i < n; i++) {
	testl	%ecx, %ecx	# n
	jle	.L9	#,
# /tmp/tmp_ed5j6ol.c:3: double matrix_multiply_asm(double* A, double* B, double* C, int n) {
	pushq	%r14	#
...
# /tmp/tmp_ed5j6ol.c:5:         for (int j = 0; j < n; j++) {
	xorl	%r9d, %r9d	# j
	.p2align 4
	.p2align 3
.L5:
# /tmp/tmp_ed5j6ol.c:6:             C[i * n + j] = 0.0;
	...
	.p2align 3
.L4:
# /tmp/tmp_ed5j6ol.c:8:                 C[i * n + j] += A[i * n + k] * B[k * n + j];
	vmovsd	(%rax), %xmm0	# MEM[(double *)_52], MEM[(double *)_52]
# /tmp/tmp_ed5j6ol.c:7:             for (int k = 0; k < n; k++) {
	addq	$8, %rax	#, ivtmp.9
# /tmp/tmp_ed5j6ol.c:8:                 C[i * n + j] += A[i * n + k] * B[k * n + j];
